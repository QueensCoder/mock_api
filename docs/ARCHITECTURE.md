...
**Why this works**
- One replication slot, one consumer, two destinations
...
Postgres RDS with `wal_level = logical`: negligible cost increase (~5–10% more WAL storage).
...
|---|---|---|
| Postgres | `postgres:16-alpine` | RDS PostgreSQL |
...
```sql
-- RDS parameter group or postgresql.conf
wal_level = logical
```

Requires a database reboot on RDS. No other Postgres changes needed.
